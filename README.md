...
可能表现为一次额外攻击或误点击。预热点击只会在 Wine 当前鼠标位置明确位于已注册
原神窗口的客户区内时发送；标题栏、窗口边框、其他 Wine 窗口或无法确认的位置都会
拒绝本次后台输入，避免触发窗口缩放或全屏。应用启动时也会在左下角日志区显示此
限制。
...
- Wine Bridge 在窗口失焦后会发送一次客户区预热点击，少数情况下可能产生一次游戏
  内误输入；无法确认点击位于游戏客户区时会拒绝后台输入。
- Wine Bridge 的后台 `inputText` 仍需继续验证；自动千星等依赖游戏文本框的脚本
  如未输入关卡名，应切回游戏前台后重试，不会回退到 CGEvent。
