...
- Wine Bridge 在窗口失焦后会发送一次客户区中心中键预热，可能使游戏视角居中；
  无法确认 Wine 光标已安全到达中心时会拒绝后台输入。
- Wine Bridge 无法确认后台 `inputText` 是否被游戏文本框消费。默认会在遇到文字输入时
  暂停脚本，等待原神保持前台 200ms 后再继续；可在启动页关闭该设置，改为明确跳过
  本次后台文字输入并继续脚本。两种策略都不会回退到 CGEvent。
