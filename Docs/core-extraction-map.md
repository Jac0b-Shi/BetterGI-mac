...
| `GameTask/AutoPathing/Handler/CombatScriptHandler.cs` | linked verbatim into Core with shared combat-scene contracts | Windows provider calls the existing `RunnerContext.GetCombatScenes`; macOS currently returns structured `CapabilityUnavailable` until the real recognition scene is composed | partial | Core verification invokes the real handler and proves `BeforeTask` plus the upstream `keydown` → `wait` → `keyup` loop. No production provider returns an empty or synthetic scene. |
