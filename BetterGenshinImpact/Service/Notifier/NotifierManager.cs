...
public class NotifierManager
{
    ...
    /// </summary>
    private void LogNotifierFailure(INotifier notifier, System.Exception ex)
    {
        if (ex is WechatClawbotSessionExpiredException)
        {
            _logger.LogWarning("微信 Clawbot 推送权限已过期，请在微信侧给 Clawbot 私聊任意内容以激活推送端口");
        }
        else
        {
            _logger.LogWarning("{name} 通知发送失败: {ex}", notifier.Name, ex.Message);
        }
    }
