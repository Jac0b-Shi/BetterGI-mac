...
public static class AvatarSpecialAction
{
    ...
    /// 资源缩放比例
    /// </summary>
    private static double AssetScale => AutoFightRuntimePlatform.Current.SystemInfo.AssetScale;
        ...
        switch (character)
        {
            ...
            case "纳西妲":
            {
                using (AvatarRecognition.BeginExclusiveOperation())
                {
                    ...
                    for (int j = 0; j < 10; j++)
                    {
                        TaskControlPlatform.Current.MoveMouseBy(1000, 0);
                        Sleep(50);
                    }
        ...
        switch (character)
        {
            ...
            case "那维莱特":
            {
                using (AvatarRecognition.BeginExclusiveOperation())
                {
                    ...
                    TaskControlPlatform.Current.SimulateAction(GIActions.NormalAttack, KeyType.KeyDown);
                    try
                    {
                        while (ms >= 0)
                        {
                            ...
                            TaskControlPlatform.Current.MoveMouseBy((int)(1000 * dpi), 0);
                            ms -= 50;
                            Sleep(50);
                        }
                    }
                    ...
                    {
                        TaskControlPlatform.Current.SimulateAction(GIActions.NormalAttack, KeyType.KeyUp);
                    }
                }
                return true;
            }
                ...
                using (AvatarRecognition.BeginExclusiveOperation())
                {
                    ...
                    try
                    {
                        ...
                        while (ms >= 0)
                        {
                            ...
                            TaskControlPlatform.Current.MoveMouseBy((int)(rateX * 50 * dpi), (int)(rateY * 50 * dpi));
                            tick = (tick + 1) % 100;
                            Sleep(25);
                            ms -= 25;
                        }
            ...
            case "桑多涅":
            {
                using (AvatarRecognition.BeginExclusiveOperation())
                {
                    ...
                    try
                    {
                        while (!avatar.Ct.IsCancellationRequested && (DateTime.UtcNow - startTime).TotalMilliseconds < maxDurationMs)
                        {
                            using (var capture = CaptureToRectArea())
                            {
                                ...
                                var drawList = new List<OpenCvSharp.Rect>();
                                ...
                                if (valid.Count > 0 && !hasLegendaryBar)
                                {
                                    ...
                                    TaskControlPlatform.Current.MoveMouseBy((int)(offsetX * 0.35 * dpi), (int)(offsetY * 0.25 * dpi));
                                ...
                                else
                                {
                                    ...
                                    if (damageResult.HasValue)
                                    {
                                        ...
                                        if (drawResults)
                                        {
                                            drawList.Add(new OpenCvSharp.Rect(dx, dy, dw, dh));
                                        }
                                    }
                                    ...
                                    if (!damageResult.HasValue)
                                    {
                                        ...
                                        if (!hasLegendaryBar && (DateTime.UtcNow - (lastSeenTargetTime ?? startTime)).TotalSeconds >= 1.5)
                                        {
                                            ...
                                            OverlayDrawPlatform.Current.SetRectangles("SandroneBloodBars", capture, drawList);
                                            break;
                                        }
                    ...
                    {
                        OverlayDrawPlatform.Current.RemoveRectangles("SandroneBloodBars");
                        TaskControlPlatform.Current.SimulateAction(GIActions.NormalAttack, KeyType.KeyUp);
                    }
                }
