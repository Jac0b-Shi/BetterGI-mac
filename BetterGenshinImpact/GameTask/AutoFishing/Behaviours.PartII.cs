...
namespace BetterGenshinImpact.GameTask.AutoFishing
{
    ...
    public partial class TurnAround : Behaviour, IScreenshotBehaviour
    {
        ...
        public TurnAround(string name, ILogger logger, IAutoFishingInput input, BgiYoloPredictor bgiYoloPredictor) : base(name)
        {
            _logger = logger;
            _input = input;
        ...
        protected async override Task<Status> Update()
        {
            ...
            if (result.Any())
            {
                ...
                _input.SetMoveBackward(true);
                sleep(100);
                _input.SetMoveBackward(false);
                sleep(400);
    ...
    public partial class EnterFishingMode : Behaviour, IScreenshotBehaviour
    {
        ...
        public EnterFishingMode(string name, ILogger logger, IAutoFishingInput input, InferenceSession session, Dictionary<string, float[]> prototypes, TimeProvider? timeProvider = null, CultureInfo? cultureInfo = null, IStringLocalizer? stringLocalizer = null) : base(name)
        {
            _logger = logger;
            _input = input;
            _session = session;
        ...
        protected async override Task<Status> Update()
        {
            ...
            if ((_pressFWaitEndTime == null || _pressFWaitEndTime < _timeProvider.GetLocalNow()) &&
                Bv.FindF(imageRegion, _fishingLocalizedString))
            {
                _input.PressInteraction();
                     ...
                     Bv.ClickWhiteConfirmButton(imageRegion))
            {
                ...
                (string? predName, _) = GridIconClassifier.Infer(resized, _session, _prototypes);
    ...
    public partial class QuitFishingMode : Behaviour, IScreenshotBehaviour
    {
        ...
        public QuitFishingMode(string name, ILogger logger, IAutoFishingInput input, CultureInfo? cultureInfo = null, IStringLocalizer? stringLocalizer = null) : base(name)
        {
            _logger = logger;
            _input = input;
            _fishingLocalizedString = stringLocalizer == null ? "钓鱼" : stringLocalizer.WithCultureGet(cultureInfo, "钓鱼");
