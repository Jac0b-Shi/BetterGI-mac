...
public partial class AutoDomainTask : ISoloTask<Dictionary<string, int>>
{
    ...
    private static string ResolveUseButtonPattern(
        ...
        IStringLocalizer<AutoDomainTask>? stringLocalizer = null)
    {
        return stringLocalizer?.WithCultureGet(cultureInfo ?? CultureInfo.GetCultureInfo("zh-Hans"), "使用")
               ?? "使用";
    }
    ...
    /// </summary>
    private static string ResolveResinNamePattern(
        string resinName,
        CultureInfo? cultureInfo = null,
        IStringLocalizer<AutoDomainTask>? stringLocalizer = null)
    {
        return stringLocalizer?.WithCultureGet(cultureInfo ?? CultureInfo.GetCultureInfo("zh-Hans"), resinName)
               ?? resinName;
    }
    ...
    public async Task<Dictionary<string, int>> Start(CancellationToken ct)
    {
        ...
        if (_useComboStrategy)
        {
            ...
            var config = AutoComboConfigPlatform.GetConfig();
