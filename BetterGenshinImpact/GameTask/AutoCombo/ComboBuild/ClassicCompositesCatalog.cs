...
namespace BetterGenshinImpact.GameTask.AutoCombo.ComboBuild
{
    ...
    {
        [Description("打开一个 Sequence（序列）作用域。序列按顺序执行子节点，全部成功才成功，任一失败则失败。用于'先做A再做B'的流程。上一次 tick 处于 RUNNING 的子节点将作为起始点，跳过前面的子节点。")]
        public Sequence Sequence([Description("节点名称")] string name, IEnumerable<Behaviour> children) => new Sequence(name, true, children);

        [Description("打开一个 Selector（选择）作用域。选择器按顺序尝试子节点，第一个成功就成功，全部失败才失败。用于'尝试A，失败则尝试B'的备选逻辑。上一次 tick 处于 RUNNING 的子节点将作为起始点，跳过前面的子节点。")]
        public Selector Selector([Description("节点名称")] string name, IEnumerable<Behaviour> children) => new Selector(name, true, children);
