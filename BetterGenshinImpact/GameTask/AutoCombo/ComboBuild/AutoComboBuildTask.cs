...
public class AutoComboBuildTask : ISoloTask
{
    ...
    private static IChatClient CreateChatClient(AutoComboBuildConfig config, ILogger logger)
    {
        ...
        try
        {
            endpoint = new Uri(config.PlanningLlmEndpoint.Trim(), UriKind.Absolute);
        }
        catch (UriFormatException e)
        {
            throw new Exception("LLM 服务地址必须是有效的绝对 HTTP/HTTPS URL", e);
        }
        ...
        if (endpoint.Scheme is not ("http" or "https") ||
            ...
            (endpoint.Scheme == Uri.UriSchemeHttp && !isLoopback))
        {
            throw new Exception("LLM 服务地址必须使用 HTTPS；HTTP 仅允许本机回环地址，且不能在 URL 中嵌入凭据");
        }
