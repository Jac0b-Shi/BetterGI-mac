...
public class GeniusInvokationControl
{
    ...
    {
        using var capture = _platform.Capture();
...
#pragma warning disable CS0612
        using var defeatedGreyMat = CaptureGameGreyMat();
        var pList = MatchTemplateHelper.MatchTemplateMulti(
            defeatedGreyMat, _assets.CharacterDefeatedMat, 0.8);
#pragma warning restore CS0612
    ...
    public void AppendCharacterStatus(Character character, Mat greyMat, int hp = -2)
    {
        ...
        // 识别角色能量
        var energyMatches = MatchTemplateHelper.FindMatches(
            characterMat,
            _assets.CharacterEnergyOnMat,
            TemplateMatchModes.CCoeffNormed,
            null,
            0.8,
            8);
        character.EnergyByRecognition = energyMatches.Count;
    ...
    public Character WhichCharacterActiveByHpOcr(Duel duel)
    {
        ...
        using var imageRegion = _platform.Capture();
        ...
        for (var i = 0; i < duel.CharacterCardRects.Count; i++)
        {
            ...
            else
            {
                ...
                text = _platform.OcrService.Ocr(activeHpMat);
