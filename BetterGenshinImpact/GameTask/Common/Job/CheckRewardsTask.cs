...
public class CheckRewardsTask
{
    ...
    {
        var captureRect = BetterGenshinImpact.GameTask.AutoFight.AutoFightRuntimePlatform.Current.SystemInfo.ScaleMax1080PCaptureRect;
    ...
    public async Task Start(CancellationToken ct)
    {
        try
        {
            if (NotificationRuntimePlatform.DragonEndSummaryEnabled)
            {
    ...
    private void ClickCommissionsButton()
    {
        using var screen = CaptureToRectArea();
        var regions = screen.FindMulti(GetConfirmRa());
        try
        {
            regions.FirstOrDefault(btn => Regex.IsMatch(btn.Text.Trim(),
                $"^(?:{Regex.Escape(_commissionsButtonString)})$", RegexOptions.IgnoreCase))?.Click();
        }
        finally
        {
    ...
    private async Task StartOriginalAsync(CancellationToken ct)
    {
        ...
        _ = await NewRetry.WaitForElementAppear(
            ...
            ()=>
            {
                TaskControlPlatform.Current.SimulateAction(GIActions.OpenAdventurerHandbook);
                ClickCommissionsButton();
            },ct,4,1000);
