    ...
    NativeDependencyStatus nativeDependencies)
{
    ...
    private async Task<RpcResponse> DispatchAsync(RpcRequest request)
    {
        ...
        try
        {
            ...
            if (request.Method == "mapMask.points.setAllHidden")
            {
                return RpcResponse.Success(
                    request.Id,
            ...
            if (request.Method == "mapMask.point.info")
            {
                return RpcResponse.Success(
                    request.Id,
                    await MapMaskRuntimePlatform.GetPointInfoAsync(
                        RequiredString(request.Params, "pointId"),
                        _shutdown.Token));
            }
            ...
            object? result = request.Method switch
            {
                ...
                "repository.update" => await _scriptRepositoryCatalog.UpdateAsync(
                    ...
                    _shutdown.Token),
                "repository.updateSubscribed" =>
                    await _scriptRepositoryCatalog.UpdateRepositoryAndSubscribedAsync(
                        RequiredString(request.Params, "channel"),
                        RequiredString(request.Params, "url"),
                        _shutdown.Token),
